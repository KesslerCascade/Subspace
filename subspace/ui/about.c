...
void showAbout(Subspace *ss)
{
    ...
    Ihandle* copyrightlbl = IupLabel(langGetC(ss, "copyright"));
    Ihandle* contactlbl   = IupLabel(langGetC(ss, "contact"));

    Ihandle* spc2 = IupSpace();
    IupSetAttribute(spc2, "SIZE", "1x12");

    Ihandle* licenselbl = IupLabel(langGetC(ss, "license"));
    ...
    Ihandle* okbutton = IupButton(langGetC(ss, "ok"), NULL);
    ...
    IupSetAttribute(dlg, "TITLE", langGetC(ss, "subspace_about"));
